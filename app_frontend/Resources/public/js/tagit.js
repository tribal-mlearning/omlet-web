    ...
    $.widget("ui.tagit", {
        ...
        _create:function () {
            ...
            this.options.select = function (event, ui) {
                // Added the behavior to put the selected metadata key into the input field, except when the user already typed the entire expression (i.e height=300) (Alan Casagrande- OnCast)
                var regex = new RegExp("(.+)(=)(.+)");
                if (regex.test(self.input.val())) return false;
                self.input.val(ui.item.value + "=")

                // Removed the behavior that add the metadata key as a tag when it is selected in the autocomplete menu (Alan Casagrande- OnCast)
            ...
            // Removed the behavior to add the metadata automatically when the input lost focus (Alan Casagrande- OnCast)
            //setup blur handler
