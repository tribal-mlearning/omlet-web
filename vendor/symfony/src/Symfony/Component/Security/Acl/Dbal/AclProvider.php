...
class AclProvider implements AclProviderInterface
{
    ...
    public function findChildren(ObjectIdentityInterface $parentOid, $directChildrenOnly = false)
    {
        ...
        $children = array();
        foreach (array_change_key_case($this->connection->executeQuery($sql)->fetchAll()) as $data) {
...
QUERY;
    ...
    private function getAncestorIds(array $batch)
    {
        ...
        foreach (array_change_key_case($this->connection->executeQuery($sql)->fetchAll()) as $data) {
        	...
        	else if(isset($data['ANCESTOR_ID']))
        		$ancestorIds[] = $data['ANCESTOR_ID'];
        	else 
        		throw new \RuntimeException('Not all ancestors ids were retrieved. This implies an implementation problem with your database driver');
        }
    ...
    {
        $ancestorIds = array_change_key_case($this->getAncestorIds($batch));
        if (!$ancestorIds) {
